...
export const STRATEGIES: Strategy[] = [
  ...
  {
    num: '02', id: 'perp-basis', zhName: '跨所资金费差套利',
    enLabel: 'PERP BASIS ARB · P0', phase: 'P0', status: 'RUNNING',
    ...
    rules: {
      entry: [
        ...
        '入场时两所价差 < 0.7%（HTX 结构溢价适配，防 adverse selection）',
        '同时持仓数小于 3 笔',
        '同一 symbol 不可重复开仓',
        'long_exchange + short_exchange 都需 trading key + perp USDT 余额',
      ],
      exit: [
        'diff_apr 衰减到 ≤ 2%（min_hold 6h 后才检查）',
        '持仓时长达到 max_hold_hours（默认 240h / 10 天兜底）',
        '跨所价格脱钩 ≥ 1.2%（最高优先级强平，防价差扩大）',
        'diff_apr 连续 3 tick 下降且 < 入场时 40% → 提前平仓',
      ],
      params: [
        { label: '入场门槛', value: 'funding diff APR ≥ 50%' },
        { label: '最少持仓', value: '6 小时' },
        { label: '最长持仓', value: '240 小时' },
        { label: '收敛平仓', value: 'diff_apr ≤ 2%' },
        { label: '价格脱钩强平', value: '|long_price - short_price| / mid ≥ 1.2%' },
        { label: '健康度分级', value: 'safe (≤200%) · risky (200-500%) · dirty (>500%)' },
        { label: '同时持仓上限', value: '3 笔' },
        { label: '单笔名义规模', value: '$100（每边 perp）' },
        { label: '永续杠杆', value: '5x（双边各 $20 margin）' },
        { label: '候选币种', value: '42 中盘 alts（排除 BTC/ETH 等主流）' },
        { label: '交易所配对', value: 'binance / okx / bybit / htx 4 家两两 = 6 pairs' },
  ...
  },
  {
    ...
    enLabel: 'CEX-DEX ARB · ARBITRUM ONE · PAPER', phase: 'P1', status: 'PAPER',
    capital: '$0', monthly: null, positions: '—', posLabel: '成交',
    desc: 'Arbitrum One 上 Uniswap V3 DEX 与 CEX（Binance）实时价差套利。Paper 模式扫描并推送信号，Live 模式双腿并发执行。',
    thesis: 'CEX 与 DEX 之间存在结构性价差：DEX 价格滞后于 CEX、大单冲击 DEX 流动性、以及跨市场套利者延迟修复。当净利润（价差 - CEX 手续费 - DEX pool fee - Arbitrum gas）超过门槛时自动执行。Arbitrum gas 极低（$0.01–0.05），提升净利可行性。',
    risks: [
      'MEV 抢跑 / 三明治攻击（Flashbots Protect RPC 可缓解）',
      'DEX 流动性瞬间枯竭导致滑点超出预期',
      '二次报价确认期间价格失效（已加 stale 检查）',
      '日损熔断后当日停止执行',
      '需要链上热钱包私钥（仅存 CEX-DEX 套利资金）',
    ],
    rules: {
      entry: [
        '净利润 ≥ $3.0（价差 - CEX fee × 2 - DEX pool fee - gas）',
        'gas_price ≤ 5 Gwei（Arbitrum 通常 0.01-0.1 Gwei）',
        '二次报价确认仍满足净利门槛',
        '当日亏损 < max_daily_loss_usd（熔断保护）',
        '开放交易数 < max_open_trades',
      ],
      exit: [
        '成交即出场（单笔原子套利，无持仓状态）',
        '任一腿失败 → 记录保守损失估计',
      ...
      params: [
        { label: '链', value: 'Arbitrum One (ChainID 42161)' },
        { label: 'DEX', value: 'Uniswap V3（QuoterV2 报价 + SwapRouter02 执行）' },
        { label: 'CEX', value: 'Binance（通过现有 adapter）' },
        { label: '候选对', value: 'ETH/USDT (pool_fee 500) · BTC/USDT (pool_fee 3000)' },
        { label: '单笔规模', value: '$200 (max_trade_usd)' },
        { label: '净利门槛', value: '$3.0 (min_net_profit_usd)' },
        { label: '滑点保护', value: '100 bps (max_slippage_bps)' },
        { label: 'gas 上限', value: '5 Gwei (max_gas_gwei)' },
        { label: '日损上限', value: '$100 (max_daily_loss_usd)' },
        { label: '报价通道', value: '$1000 名义额向 QuoterV2 查价，减少精度损失' },
        { label: '扫描间隔', value: '5 秒' },
        { label: '执行模式', value: 'paper（可在配置面板切 live）' },
      ],
    },
  },
  {
