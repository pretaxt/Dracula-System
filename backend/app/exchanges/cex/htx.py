"""HTXAdapter — HTX (Huobi) USDT swap perpetual + spot 适配器。"""
...
class HTXAdapter(CCXTAdapter):
    def __init__(
        ...
        if testnet:
            try:
                ...
                logger.info("htx_testnet_mode")
            except Exception:
                logger.debug("htx_testnet_unsupported")
        self._clients = {
    ...
    async def fetch_spot_margin_usdt_balance(self) -> "Decimal":
        ...
        try:
            ...
            logger.debug("htx_fetch_spot_balance_failed", error=str(exc)[:200])
            return _D("0")
    ...
    async def fetch_order(self, order_id: str, symbol: Symbol):  # type: ignore[override]
        raise NotImplementedError("HTXAdapter is read-only in market-data mode")
