...
- **#3 跨所价差套利** 从"永久排除"改为"**保留**"
- **#12 因子模型** 从"P1+ 候选"改为"**砍掉**"
- **#17 MEV 抢跑** 保持砍掉(本次决策中老虎采纳风险提示,维持永久排除)

变更后:
- 保留 12 个:#1 #2 #3 #4 #5 #6 #7 #9 #10 #13 #14 #16
- 砍掉 5 个:#8 #11 #12 #15 #17(不再展示、不再纳入文档/UI)

#### UI 原型迭代

- `dashboard_v1.0_5.html` 完成(在 v1.0 基础上演进)
  - 颜色 token 更新(血色 #e34058 / 浅色 #b81a30)
  - 字体引入 Cinzel(display)与 Cormorant Garamond(serif)
  - 6 视图结构稳定、双主题、双语切换 i18n 字典完整

#### 前端重做计划启动

- 基线从 v0.3(绿色主题)切换到 v1.0_5(血色吸血鬼主题)
- 既有 frontend 代码(Next 14.2.35 App Router)保留,UI 层全部重做
- 后端 API 保持不变
